...
int Sampler::sampleTopK(const int *indices, const float *logits, int k) {
    if (indices == nullptr || logits == nullptr || k <= 0)
        throw std::runtime_error("Invalid top-k logits payload");
    ...
    for (int i = 0; i < k; i++) {
        const float scaled = logits[i] * invTemp;
        if (scaled > maxLogit)
            maxLogit = scaled;
    }
    ...
    float sum = 0.0f;
    for (int i = 0; i < k; i++) {
        probs[i] = expf(logits[i] * invTemp - maxLogit);
        sum += probs[i];
    }
    if (sum <= 0.0f)
        return indices[0];
    ...
    const float coin = randomF32(&rngState);
    if (topp <= 0.0f || topp >= 1.0f) {
        const int local = sample_mult(probs.data(), k, coin);
    ...
    for (int i = 0; i <= cutoff; i++) {
        cdf += probs[order[i]];
        if (target <= cdf)
            return indices[order[i]];
    }
    return indices[order[cutoff]];
}
