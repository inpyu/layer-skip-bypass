...
public:
    ...
    void addSync(NnUint pipeIndex, NnSyncType syncType) {
        NnSyncConfig sc{};
        sc.pipeIndex = pipeIndex;
        sc.syncType = syncType;
        syncs.push_back(sc);
    }

    void addSpKvSync(NnUint positionPipeIndex, NnUint kBufferIndex, NnUint vBufferIndex,
                     NnUint localSeqStart, NnUint localSeqLen, NnUint kvDim0) {
        NnSyncConfig sc{};
        sc.pipeIndex = positionPipeIndex;
        sc.syncType = SYNC_SP_KV;
        sc.spKvKBufferIndex = kBufferIndex;
        sc.spKvVBufferIndex = vBufferIndex;
        sc.spKvLocalSeqStart = localSeqStart;
        sc.spKvLocalSeqLen = localSeqLen;
        sc.spKvDim0 = kvDim0;
        sc.spKvPositionPipeIdx = positionPipeIndex;
        syncs.push_back(sc);
    }
